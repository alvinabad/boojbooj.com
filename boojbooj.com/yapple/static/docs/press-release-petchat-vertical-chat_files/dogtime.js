...
function toggle_breeds() {
    if ($F('want_mixed')=='mixed' || $F('want_purebred')=='purebred') {
        $$('#breeds input').each(function(cb) {cb.disabled=false;});
        $$('#non_recommended_breeds input').each(function(cb) {cb.disabled=false;});
    } else {
        $$('#breeds input').each(function(cb) {cb.disabled=true;});
        $$('#non_recommended_breeds input').each(function(cb) {cb.disabled=true;});        
    }
}

function populate_user_form() {
    $('user_name').value=$('email').value.split('@')[0];
    $('user_email').value=$('email').value;
    $('user_password').value=$('zip').value;
    $('user_password_confirmation').value=$('zip').value;
}
...
var hideNav = false;		// Hide Next/Prev buttons by default?
...
Lightbox.prototype = {
	...
	initialize: function() {	
		...
		objImageContainer.setAttribute('style', 'padding-top:'+borderSize+'px');
	...
	changeImage: function(imageNum) {	
		...
		imgPreloader.onload=function(){
			...
			$('lightboxImage').setAttribute('width', imgPreloader.width);
			$('lightboxImage').setAttribute('height', imgPreloader.height);
...
var EditForm = {
  ...
  cancel: function() {
		var currentId = this.currentReplyId();
		if(currentId != null){
			$('post-body-' + currentId + '-edit').update('');
			$('post-body-' + currentId).show();
		}
    this.clearReplyId();
  }
}
...
//
// Lifted from http://gorondowtl.sourceforge.net/wiki/Cookie on 24 Jan 2008
...
var Cookie = {
  ...
  get: function(name) {
    ...
    return (cookie ? unescape(cookie[2]).replace( /\+/g, ' ') : null);
  },
  erase: function(name) {
...
function gotoSelection(sel) {
    ...
    if ( url != "" ) location.href = sel.options[sel.selectedIndex].value
    return ( url == "" )
}
